...
app.listen(hardcoded_port, () => {
  ...
  if (String(hardcoded_port) !== String(real_port)) {
    console.log(
      `[LỖI] Cổng ${hardcoded_port} KHÔNG ĐÚNG với tài khoản của bạn!`,
    );
  ...
  core
    .submit(MY_NAME)
    .then((response) => {
      console.log("-> Đã gửi thành công! Hãy tải lại Bảng Xếp Hạng!");
    })
    .catch((error) => {
      console.log("-> Gửi thất bại, lỗi mạng hoặc server!");
    });
});
